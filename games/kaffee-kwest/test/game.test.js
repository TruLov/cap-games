...
test('publicState never leaks the tree, the seed or vote choices', () => {
  ...
  game.applyMove(state, { type: 'vote', option: 1 }, 'alice');
  ...
  assert.deepEqual(pub.votes, { alice: true });                // who voted, not what
  ...
  assert.equal(pub.casting.alice.hook, undefined);             // hooks are private
});

test('privateState adds the own role hook on top of the public view', () => {
  const state = game.init(settings());
  const priv = game.privateState(state, 'alice');
  assert.equal(priv.me.role, state.casting.alice.role);
  assert.equal(priv.me.hook, state.casting.alice.hook);
...
test('score: co-op draw for everyone, points by ending tier', () => {
  const players = [...party.map(p => ({ ...p })), { spectator: true, user: 'sam' }];
...
test('chronicler suggests at most 2 scenario-neutral entries from the log', () => {
  const finalState = {
    party,
    casting: { alice: { role: 'Schmied:in mit Vorschlaghammer' }, bob: { role: 'Unterhändler:in des Dorfes' } },
    log: [
      { kind: 'roll', by: 'alice', die: 20, bonus: 0, total: 20, target: 12, success: true },
      { kind: 'moment', by: 'alice', text: 'Ich biete dir meinen Hammer!' },
      { kind: 'roll', by: 'alice', die: 1, bonus: 2, total: 3, target: 12, success: false },
      { kind: 'roll', by: 'bob', die: 10, bonus: 0, total: 10, target: 12, success: false },
    ],
  };
