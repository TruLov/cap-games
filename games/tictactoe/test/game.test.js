...
const play = (cells, first = 'X') => {
  let s = game.init({ firstPlayer: first }, players), r;
...
describe('tictactoe', () => {
  ...
  it('interface + init contract', () => {
    ...
    const s = game.init({ firstPlayer: 'O' }, players);
    expect(s.board).to.have.length(9);
    expect(s.turn).to.equal(O);              // O moves first → bob
    expect(s.marks).to.deep.equal({ alice: 'X', bob: 'O' });
  });

  it('applyMove rejects illegal moves', () => {
    const s = game.init({ firstPlayer: 'X' }, players);
  ...
  it('win → end.winner is the user', () => {
    expect(play([0, 3, 1, 4, 2]).end?.winner).to.equal(X); // top row, X (alice)
  });
